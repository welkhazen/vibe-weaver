...
## 2025-05-16 - Optimized SVG Theme Reactivity
**Learning:** Even simple SVG icons can cause performance issues if they rely on JavaScript-based theme polling (like `MutationObserver` + `getComputedStyle`). This pattern causes layout thrashing and unnecessary React re-renders.
**Action:** Use `currentColor` within SVGs and set the `color` property on the parent component using CSS variables. This leverages the browser's native style engine for reactivity.
