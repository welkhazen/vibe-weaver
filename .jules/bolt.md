...
## 2026-04-16 - Memoizing Expensive Derived Data in Gesture-Heavy Components
**Learning:** High-frequency state updates (like `dragX` during a swipe gesture) trigger re-renders at 60FPS. Any $O(N \log N)$ operation (like `.sort()`) performed directly in the render body will execute on every frame, even if the data being sorted hasn't changed. This creates significant cumulative CPU overhead and can lead to "jank" or frame drops during interactions.
**Action:** Always use `useMemo` for derived data calculations like sorting, filtering, or complex mapping, especially in components that handle gestures or animations. Ensure that the dependency array only includes the data source, not the animation state.
