  ...
  useEffect(() => {
    ...
    const calculateOpacity = (now: number) => {
      ...
      if (elapsed < fadeStartTime) {
        ...
        const easeOutVal = 1 - Math.pow(fadeProgress, 2);
        const baseOpacity = isDarkRef.current ? 0.3 : 0.2;
        return baseOpacity * easeOutVal;
      } else {
        return 0;
      }
    };
    ...
    const getCurrentInterval = (now: number) => {
      ...
      return speedRef.current + (targetSpeedRef.current - speedRef.current) * easeOutVal;
    };
    ...
    const loop = (now: number) => {
      ...
      const shouldContinueVal = updateCanvasOpacity(now);
      if (!shouldContinueVal) {
