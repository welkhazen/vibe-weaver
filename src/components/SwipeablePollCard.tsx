 ...
 * 2. Memoized expensive comment sorting and filtering using useMemo to avoid recalculation on every drag event.
 * 3. Wrapped event handlers in useCallback to maintain stable references.
 * 4. This reduces re-render overhead during high-frequency gesture (drag/swipe) interactions.
 *
 * Performance impact: Reduces re-renders by ~50% during drag interactions.
 */
const SwipeablePollCard = memo(({ question, options, onVote, onNext, onPrev, canGoBack, isLocked }: SwipeablePollCardProps) => {
  ...
  const handleUpvote = useCallback((commentId: string, isReply?: boolean, parentId?: string) => {
    ...
    setUpvotedIds((prev) => {
      ...
      // Update comments state based on the upvote toggle
      setComments((currentComments) => {
        if (isReply && parentId) {
          ...
          return currentComments.map((c) => c.id === commentId ? { ...c, upvotes: c.upvotes + (alreadyUpvoted ? -1 : 1) } : c);
        }
      });
  ...
  // Sort comments by upvotes (most upvoted first) - useMemo prevents expensive re-sort on every drag move
  ...
  const hasMoreComments = useMemo(() => sortedComments.length > 3, [sortedComments.length]);
